...
/// (buzz#33), appending the `["encrypted", "nip44-v2", keyId]` marker tag
/// right after the caller's `["h", channelId]` tag — same relative position
/// `channelMessageCrypto.ts`'s `publishMessage` uses. Returns the content to
/// actually put on the event: unchanged for an unkeyed channel (the common
/// case), sealed ciphertext for a keyed one.
///
/// Every channel-scoped builder that carries user-authored `content` must
/// call this immediately after pushing its `h` tag and before adding any
/// other tag or constructing the `EventBuilder` — this is the one seam that
/// stands between a Rust-built event and leaving the process in the clear.
fn seal_for_channel(
    channel_id: Uuid,
    content: &str,
    tags: &mut Vec<Tag>,
) -> Result<String, String> {
    let sealed = crate::channel_keys::seal_for_channel(&channel_id.to_string(), content);
    if let Some([scheme_tag, scheme, key_id]) = &sealed.tag {
        tags.push(tag(vec![scheme_tag, scheme, key_id])?);
    }
    Ok(sealed.content)
}
...
pub fn build_message_with_client_tags(
    ...
    let mut tags = vec![tag(vec!["h", &channel_id.to_string()])?];
    let content = seal_for_channel(channel_id, content, &mut tags)?;
...
pub fn build_message_edit(
    ...
    let mut tags = vec![tag(vec!["h", &channel_id.to_string()])?];
    let content = seal_for_channel(channel_id, content, &mut tags)?;
    tags.push(tag(vec!["e", &target_event_id.to_hex()])?);
    tags.extend(mention_tags(mentions)?);
    imeta_tags(media_tags, &mut tags)?;
    emoji_tags(custom_emoji_tags, &mut tags)?;
