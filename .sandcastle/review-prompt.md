...
4. **Check correctness**:
   ...
   - Are there unsafe casts, `any` types, unchecked `unwrap()`/`expect()` on fallible paths, or unchecked assumptions?
...
readability, redundant code, formatting, inconsistency with neighbouring code —
that preserve exact behaviour.
...
- the diff asserts something factually wrong (a comment, doc, or test that
  misstates the actual behaviour) that you cannot verify a fix for
